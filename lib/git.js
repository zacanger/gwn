...
const tarProgram = isProgramInstalled('gtar') ? 'gtar' : 'tar'
...
const downloadRex = /\/api\/([^"&'/<>`|]*)\.([.2abgiprtxz]*)$/
...
function archive(req, res) {
  ...
  const tempfile =
    '/tmp/' + new Date().getTime() + '_' + repo + '-' + Math.random() + '.zip'
...
function listFiles(req, res) {
  ...
  exec(cmdline, opts, (err, stdout) => {
    ...
    for (const element of split) {
      const dta = element.split(/\s+/)
      if (/^([drswx-]{10})/.test(dta[0])) {
...
function list(req, res) {
  ...
  const sortrepos = (a, b) => (a.name <= b.name ? -1 : 1)
