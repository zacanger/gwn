...
app.get('/', (req, res) => {
  request
    .getJson(`${apiUrl}/list`)
    .then((repos) =>
      repos.map(({ name, lastCommit: { age, message, author } }) => ({
        name,
        age,
        message,
        author,
...
app.get('/:repo', (req, res) => {
  ...
  request.getJson(`${apiUrl}/${repo}?skip=${skip}`).then((commits) => {
...
app.get('/:repo/commit/:hash', (req, res) => {
  ...
  request.sendString('GET', `${apiUrl}/${repo}/${hash}`).then((commit) => {
...
app.get('/:repo/files', (req, res) => {
  const repo = req.params.repo
  request.getJson(`${apiUrl}/${repo}/list`).then((files) => {
...
app.get('/:repo/file/:file', (req, res) => {
  ...
  request
    .sendString('GET', `${apiUrl}/${repo}/get/${file}?raw=true`)
    .then((content) => {
