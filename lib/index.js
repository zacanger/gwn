...
// ui routes
const red = (req, res) => { res.redirect('/git/') }
app.get('', red)
app.get('/', red)
app.get(/\/git$/, red)
app.get(/\/git\/index.html/, getFile('index.html'))
app.get(/\/git\/?$/, getFile('index.html'))

// api routes
app.get('/git/list', list)
app.get(downloadRex, archive)
app.get(/\/git\/[^\/`'"'&|<>]*$/, log)
app.get(/\/git\/[^\/`'"'&|<>]*\/[abcdef1234567890]*$/, diff)
app.get(/\/git\/[^\/`'"'&|<>]*\/list$/, listFiles)
app.get(listFileRex, getOneFile)
